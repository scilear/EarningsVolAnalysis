...
<head>
  <meta charset="utf-8" />
  <title>{{ ticker }} Earnings Volatility Report</title>
  <style>
...
<body>
  <h1>{{ ticker }} Earnings Volatility Report</h1>
  ...
  <table>
    ...
    <tr>
      ...
      <td>{{ "%.4f" | format(row.capital_normalized_ev) }}</td>
  ...
  <div class="trade-sheet">
    ...
    <table>
      ...
      {% endif %}
      <tr>
        <th>Capital-Normalized EV</th>
        <td>{{ "%.4f" | format(strat.capital_normalized_ev) }}</td>
        <th>EV</th>
        <td>${{ "%.2f" | format(strat.ev) }}</td>
      </tr>
      <tr>
  ...
  <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #e2e8f0; color: #718096; font-size: 0.9em; text-align: center;">
    <p>{{ ticker }} Earnings Volatility Analysis Report | Generated by automated regime classification engine</p>
