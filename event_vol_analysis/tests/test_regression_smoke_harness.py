...
def test_resolve_tests_prefers_explicit_selection() -> None:
    explicit = ["event_vol_analysis/tests/test_alignment.py"]
...
def test_build_pytest_command_defaults_to_quiet_mode() -> None:
    command = build_pytest_command(
        ...
        tests=["event_vol_analysis/tests/test_alignment.py"],
        verbose=False,
        fail_fast=False,
        extra_pytest_args=None,
    )
    assert command[:4] == ["python3", "-m", "pytest", "-q"]
    assert "event_vol_analysis/tests/test_alignment.py" in command
